...
# read tree file
tree = Phylo.read("V49-Midpoint-rooted-Tree_change_name.NWK", "newick")
# name internal nodes
...
with open("tree_rename.csv", mode='w', newline='') as file:
    ...
    writer.writerow(["Precedent", "consequent", "weight", "height"])
...
#V49_data=pd.read_csv("V49_raw_data_with_COG_tax_only_species.csv", sep = ',')
    ...
    for b in range(2, column_len):
        ...
        for t in first_column_value:
            if t=='Node114':#set the weight of root
                ...
                weight_taxon=weight.loc[weight['consequent']==t]['height'].iloc[0]
