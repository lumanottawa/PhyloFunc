# The pipeline of the PhyloFunc distance calculation. The code includes 4 Steps: preprocession of data, the construction of branches,creation of extended taxon composition table and PhyloFunc distance calculation  
### original datafiles in the folder 1_datasets_search_results\3_human gut microbiome:
# Step1: preprocess the raw data to creat taxon-function table
## proteinGroups.txt, taxon.tsv, function.csv are generated by database search using MetaLab
import pandas as pd
from collections import Counter
# match the raw data from proteinGroups.txt,taxon.tsv and function.csv to get taxon-function table
# read protein_group tabe and filter any protein group that includes REV_ in the id names
protein_group = pd.read_table("proteinGroups.txt", sep = "\t")
protein_group = protein_group[~protein_group['Protein IDs'].str.contains('REV_')]
# expand protein groups by semicolon
...
#match taxon and function table to creat taxon-function table
...
# merge taxon_cog table with the protein data.
...
#merge all of levels to get taxon-function table
Protein_phylum_unique_index = Protein_phylum_unique['ProteinGroupIDs']
...
#Step2 construct the branches table of tree
## This code is for generating PhyloFunc distace for human gut microbiome dataset
## tree of V49-Midpoint-rooted-Tree.NWK was constructed by GTDB-Tk
from Bio import Phylo
import pandas as pd
import csv
#read tree file and tree name file
#read tree name file
...
#Step3 calculate tax_composition, function composition and weighted_function_composition according to Taxon-Function Table
#read Taxon-Function Table Protein_LCA_species_COG_sum_V49_LFQ.csv, that has been preprocessed at step1
#generate tree_branches,tax_composition,weighted_function_composition.csv,weighted_function_composition_percentage.csv, extend_taxon_composition_merge_all_nodes.csv automatically during running the code to verify the calculation process of the program.
...
#expand to represent all nodes up to the root of the phylogeny through summing up each node,
...
# Step4 calculate PhyloFunc distance basd on tables "weighted_function_composition_percentage" and "extend_taxon_composition_merge_all_nodes"
